...
from loss.loss_term_builder import StandardLossTermBuilder
...
"""
Trainables - Deep Regression w. Metrics
-------------------------------------------------------------------------------------------------------------------------------------------
"""
def deep_regr_add_metrics(config, dataset: TensorDataset, exp_cfg: ExperimentConfig):
    ...
    lt_builder = StandardLossTermBuilder()
    add_eval_metrics = {name: lt_builder.create_for_Regr(name) for name in exp_cfg.eval_metrics}

    eval_cfg = EvalConfig(data_key = 'labelled', output_name = 'regr_iso', mode = 'iso', loss_name = optim_loss)
    visitors = [
        RegrOutputVisitor(eval_cfg = eval_cfg),
        RegrLossVisitor(regr_loss_term, eval_cfg = eval_cfg),
        LossTermVisitor(loss_terms = add_eval_metrics, eval_cfg = eval_cfg)
    ]
    ...
    train.report({optim_loss :loss_regr, **{name: results.metrics[name] for name in exp_cfg.eval_metrics}})
