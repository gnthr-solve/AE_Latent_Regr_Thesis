...
class GlobalBestModelSaver(Callback):
    ...
    def on_trial_complete(self, iteration, trials: list[Trial], trial: Trial, **info):
        ...
        with threading.Lock():
            self._on_trial_complete(iteration = iteration, trials = trials, trial = trial, **info)


    def _on_trial_complete(self, iteration, trials: list[Trial], trial: Trial, **info):
                ...
                if checkpoint:
                    ...
                    with checkpoint.as_directory() as checkpoint_dir:
                        ...
                        for src_model_path in model_files:
                            ...
                            logger.debug(f"Copy of {src_model_path} to {dest_model_path} complete")
    ...
    def _cleanup_checkpoints(self, trials: list[Trial], exclude_trial: Trial):
        ...
        for t in trials.copy(): #NOTE originally just trials, attempt trials.copy() to avoid including trials that terminate during iteration
...
"""
Ray Callbacks - GlobalBestModelSaver_Previous
-------------------------------------------------------------------------------------------------------------------------------------------
Store in case modifications are problematic
"""
class GlobalBestModelSaver_Previous(Callback):
